...
SceneRecorder2::SceneRecorder2(char *filename)
{
	strcpy(dir, filename);
...
void SceneRecorder2::newFrame(const yohan::base::DATA timeStamp)
{
	char tmp[16];
...
void SceneRecorder2::record(VolumeModel2* model)
{
	model->output(sceneFile, frameFileName, indexOfFrame);
}

void SceneRecorder2::endScene()
{
	this->sceneFile = fopen(sceneFileName, "a+");
